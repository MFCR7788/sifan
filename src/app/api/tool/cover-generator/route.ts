...
function buildPrompt(platform: string, style: string, text: string): string {
  ...
  return `[核心任务]生成一张高质量的封面图片，风格：${styleDescriptions[style]}，用途：${platformDescriptions[platform]}。

[主题内容]${text}

[详细设计要求]
1. 【质量要求】高质量高清图片，细节丰富，专业摄影级别画质
2. 【构图要求】构图均衡，视觉焦点突出，主体鲜明，符合${style}风格的构图特点
3. 【色彩要求】色彩搭配和谐，严格按照${style}风格的色调要求执行，避免色彩冲突
4. 【尺寸适配】适合作为${platform}的封面图使用，比例协调，符合平台规范
5. 【风格强化】整体视觉效果必须体现${style}风格的特征，不能偏离
6. 【禁止事项】**绝对不要在图片中添加任何文字、标题、水印、字母或符号，仅使用纯视觉元素**
7. 【背景处理】背景简洁不喧宾夺主，与主体形成良好对比，突出主题
8. 【视觉吸引力】具有强烈的视觉吸引力和冲击力，能够在${platform}上脱颖而出
9. 【元素表达】仅使用图像、图形、色彩、光影、纹理等纯视觉元素来表达主题

[关键提示]
- 优先级：${style}风格 > 平台适配 > 主题表达
