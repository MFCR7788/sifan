	...
	const fetchTransactions = async () => {
		...
		try {
			const url = filter === 'all'
				? '/api/admin/transactions?type=recharge'
				: `/api/admin/transactions?type=recharge&status=${filter}`;
	...
	return (
		<div>
			<div className="flex items-center justify-between mb-8">
				<div>
					<h1 className="text-3xl font-bold text-gray-900">充值明细</h1>
					<p className="text-sm text-gray-600 mt-1">查看所有会员充值交易记录</p>
				</div>
			</div>
			...
			{loading ? (
				...
				<div className="bg-white rounded-xl p-12 border border-gray-200 text-center">
					<p className="text-gray-500">{searchTerm ? '未找到匹配的交易记录' : '暂无充值记录'}</p>
				</div>
			) : (
					...
					<div className="bg-white rounded-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
						<div className="p-6 border-b border-gray-200">
							<div className="flex items-center justify-between">
								<h2 className="text-xl font-semibold text-gray-900">充值详情</h2>
								<button
						...
						<div className="p-6">
							<div className="space-y-4">
								...
								<div className="grid grid-cols-2 gap-4">
									<div>
										<div className="text-sm text-gray-600">充值金额</div>
										<div className="font-bold text-green-600 text-xl">+¥{formatAmount(selectedTransaction.amount)}</div>
									</div>
									<div>
								...
								{selectedTransaction.description && (
									<div>
										<div className="text-sm text-gray-600">说明</div>
