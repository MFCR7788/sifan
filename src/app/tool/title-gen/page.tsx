...
export default function TitleGenPage() {
  ...
  const [history, setHistory] = useState<Array<{ original: string; titles: string[]; platform: string; timestamp: Date }>>([]);
  const textareaRef = useRef<HTMLTextAreaElement>(null);

  const platforms = ['小红书', '公众号', '抖音', '视频号', '快手'];
  const maxChars = 2000;
  ...
  const handleGenerate = async () => {
    ...
    try {
      // 调用API进行标题生成
      const response = await fetch('/api/tool/title-gen', {
        method: 'POST',
        headers: {
      ...
      const newRecord = {
        original: inputText,
        titles: data.titles || data.result || [],
        platform: selectedPlatform,
        timestamp: new Date(),
      };
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sm:p-8">
          ...
          {/* 文本输入框 */}
          <div className="mb-6">
            <h2 className="text-sm font-medium text-gray-900 mb-3">输入正文或关键词</h2>
            <div className="relative">
              <textarea
                ref={textareaRef}
                value={inputText}
                onChange={(e) => setInputText(e.target.value)}
                placeholder="请输入正文内容或关键词，将自动生成5个吸引人的标题..."
                maxLength={maxChars}
                className={`
        ...
        {/* 生成标题历史记录 */}
        <div className="mt-8">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">生成标题历史记录</h2>
          {history.length === 0 ? (
            ...
            <div className="space-y-4">
              {history.map((record, index) => (
                ...
                >
                  ...
                  {/* 原始输入 */}
                  <div className="mb-4">
                    <p className="text-xs text-gray-500 mb-1">原始内容：</p>
                  ...
                  {/* 生成的标题列表 */}
                  <div>
                    <p className="text-xs text-gray-500 mb-2">生成的标题：</p>
                    <div className="space-y-2">
                      {record.titles.map((title, titleIndex) => (
                        <div
                          key={titleIndex}
