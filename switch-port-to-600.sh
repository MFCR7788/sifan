...
sed -i "s/(${NEW_PORT}端口)/(${NEW_PORT}端口)/g" ${NGINX_CONF}
...
echo ""
echo "现在请访问："
echo "  - http://42.121.218.14:${NEW_PORT}"
echo "  - http://www.zjsifan.com:${NEW_PORT}"
echo "  - http://zjsifan.com:${NEW_PORT}"
echo ""
echo "⚠️  重要：需要在阿里云安全组开放 ${NEW_PORT} 端口"
echo ""
echo "安全组配置："
echo "  - 协议类型：自定义TCP"
echo "  - 端口范围：${NEW_PORT}/${NEW_PORT}"
echo "  - 授权对象：0.0.0.0/0"
echo ""
echo "配置安全组后，即可通过域名访问"
